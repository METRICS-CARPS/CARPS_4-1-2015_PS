...
No pre-procesing needed other than tidying above.
...
ggplot() + 
  geom_line(data = desc_stat, aes(x = dft_value, y = attractive_mean), color = "green") + 
    geom_errorbar(aes(x = desc_stat$dft_value, 
                      ymin = desc_stat$attractive_mean - desc_stat$attractive_sd/sqrt(528), 
                      ymax = desc_stat$attractive_mean + desc_stat$attractive_sd/sqrt(528)), 
                  width=.1, position = pd)  + 
  geom_line(data = desc_stat, aes(x = dft_value, y = trust_mean), color = "blue") +
  geom_errorbar(aes(x = desc_stat$dft_value, 
                    ymin = desc_stat$trust_mean - desc_stat$trust_sd/sqrt(528), 
                    ymax = desc_stat$trust_mean + desc_stat$trust_sd/sqrt(528)), width=.1,
position = pd)  +  
  xlab('DFT Value') + 
  ylab('Mean Judgment') + 
  ylim(0, 10) + 
